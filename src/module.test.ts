...
jest.mock('@grafana/runtime', () => {
  // Initialize in the factory function so they exist when mocks are called
  const handlers: any[] = [];
  const reportInteractions: any[] = [];

  // Store in global for test access
  (global as any).testHandlers = handlers;
  (global as any).testReportInteractions = reportInteractions;

  return {
    ...jest.requireActual('@grafana/runtime'),
    reportInteraction: (...args: any[]) => reportInteractions.push(args),
    getAppEvents: () => ({
...
describe('queriesOnInitDashboard', () => {
  it('should report a grafana_elasticsearch_dashboard_loaded interaction ', () => {
    const handlers = (global as any).testHandlers;
    const reportInteractions = (global as any).testReportInteractions;

    // Trigger the event through the subscribed handler
    handlers[0](
    ...
    expect(reportInteractions).toHaveLength(1);
    expect(reportInteractions[0]).toEqual([
      'grafana_elasticsearch_dashboard_loaded',
      {
